...
class TeleVisionWrapper:
    ...
    def get_data(self):
        ...
        logging.info("TeleVisionWrapper.get_data() started")

        # --------------------------------wrist-------------------------------------

        # TeleVision obtains a basis coordinate that is OpenXR Convention
        head_vuer_mat, head_flag = mat_update(
            const_head_vuer_mat, self.tv.head_matrix.copy()
        )
        left_wrist_vuer_mat, left_wrist_flag = mat_update(
            const_left_wrist_vuer_mat, self.tv.left_hand.copy()
        )
        right_wrist_vuer_mat, right_wrist_flag = mat_update(
            const_right_wrist_vuer_mat, self.tv.right_hand.copy()
        )

        logging.info(
            f"Wrist flags - left: {left_wrist_flag}, right: {right_wrist_flag}"
        )
        logging.debug(f"Raw head matrix: {self.tv.head_matrix}")
        logging.debug(f"Raw left hand matrix: {self.tv.left_hand}")
        logging.debug(f"Raw right hand matrix: {self.tv.right_hand}")
        ...
        logging.debug(f"After convention change - head matrix: {head_mat}")
        logging.debug(f"After convention change - left wrist: {left_wrist_mat}")
        logging.debug(f"After convention change - right wrist: {right_wrist_mat}")

        # Change wrist convention: WristMat to UnitreeWristMat
        logging.info("Converting to Unitree wrist convention")

        unitree_left_wrist = left_wrist_mat @ (
            T_to_unitree_left_wrist if left_wrist_flag else np.eye(4)
        )
        unitree_right_wrist = right_wrist_mat @ (
            T_to_unitree_right_wrist if right_wrist_flag else np.eye(4)
        )

        logging.debug(f"After Unitree conversion - left wrist: {unitree_left_wrist}")
        logging.debug(f"After Unitree conversion - right wrist: {unitree_right_wrist}")

        # Transfer from WORLD to HEAD coordinate (translation only)
        logging.info("Transforming coordinates from WORLD to HEAD")
        ...
        logging.debug(
            f"After world-to-head transform - left wrist: {unitree_left_wrist}"
        )
        logging.debug(
            f"After world-to-head transform - right wrist: {unitree_right_wrist}"
        )
        ...
        #    z0 z1 z2 ··· z23 z24
        #     1  1  1 ···   1   1
        logging.info("Processing hand data")
        ...
        logging.debug(f"left_hand_vuer_mat shape: {left_hand_vuer_mat.shape}")
        logging.debug(f"left_hand_vuer_mat sample cols:\n{left_hand_vuer_mat[:, :3]}")
        logging.debug(f"right_hand_vuer_mat shape: {right_hand_vuer_mat.shape}")
        logging.debug(f"right_hand_vuer_mat sample cols:\n{right_hand_vuer_mat[:, :3]}")
        ...
        logging.debug(f"T_robot_openxr:\n{T_robot_openxr}")
        logging.debug(f"left_hand_mat shape: {left_hand_mat.shape}")
        logging.debug(f"left_hand_mat sample cols:\n{left_hand_mat[:, :3]}")
        ...
        logging.debug(f"left_hand_mat_wb shape: {left_hand_mat_wb.shape}")
        logging.debug(f"left_hand_mat_wb sample cols:\n{left_hand_mat_wb[:, :3]}")
        ...
        logging.debug(f"unitree_left_hand shape: {unitree_left_hand.shape}")
        logging.debug(f"unitree_left_hand sample rows:\n{unitree_left_hand[:3, :]}")

        # --------------------------------offset-------------------------------------
        logging.info("Applying offsets")

        head_rmat = head_mat[:3, :3]
        # The origin of the coordinate for IK Solve is the WAIST joint motor.
        unitree_left_wrist[0, 3] += 0.15
        unitree_right_wrist[0, 3] += 0.15
        unitree_left_wrist[2, 3] += 0.45
        unitree_right_wrist[2, 3] += 0.45

        logging.info(f"Final left wrist height (z): {unitree_left_wrist[2, 3]:.3f}")
        logging.info(f"Final right wrist height (z): {unitree_right_wrist[2, 3]:.3f}")
        logging.info("TeleVisionWrapper.get_data() completed")
