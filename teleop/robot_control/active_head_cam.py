...
# class ActiveCameraThread:
#     """Threaded active camera controller that runs head tracking independently"""
    
#     def __init__(self, tv_wrapper, camera_controller, safe_mode=True, max_movement_deg=60.0, logger=None):
#         self.tv_wrapper = tv_wrapper
#         self.camera_controller = camera_controller
#         self.safe_mode = safe_mode
#         self.max_movement_rad = np.radians(max_movement_deg)
#         self.logger = logger or logging.getLogger('camera_thread')
...
#         self.start_pitch_rad = 0.0
#         self.start_yaw_rad = 0.0
        
#     def start(self):
#         """Start the camera control thread"""
#         if self.thread is not None and self.thread.is_alive():
#             self.logger.warning("Camera thread already running")
#             return False
...
#         # Get initial camera positions
#         if self.camera_controller and self.camera_controller.connected:
#             self.start_pitch_rad, self.start_yaw_rad = self.camera_controller.start_positions
#             with self.state_lock:
#                 self.current_pitch = self.start_pitch_rad
#                 self.current_yaw = self.start_yaw_rad
#                 self.target_pitch = self.start_pitch_rad
#                 self.target_yaw = self.start_yaw_rad
        
#         self.logger.info(f"Initial camera positions: Pitch={np.rad2deg(self.start_pitch_rad):.2f}°, Yaw={np.rad2deg(self.start_yaw_rad):.2f}°")
        
#         # Start the control thread
#         self.running = True
#         self.thread = threading.Thread(target=self._control_loop, daemon=True)
#         self.thread.start()
#         self.logger.info("Camera control thread started")
#         return True
...
#             self.thread = None
#         self.logger.info("Camera control thread stopped")
    
#     def get_positions(self):
#         """Get current camera positions (thread-safe)"""
#         with self.state_lock:
#             return self.current_pitch, self.current_yaw
    
#     def get_target_positions(self):
#         """Get current target positions (thread-safe)"""
#         with self.state_lock:
#             return self.target_pitch, self.target_yaw
    
#     def _control_loop(self):
#         """Main camera control loop running in separate thread"""
#         try:
#             while self.running:
#                 if not (self.camera_controller and self.camera_controller.connected):
#                     time.sleep(0.02)
#                     continue
...
#                 target_pitch_rad = self.start_pitch_rad + np.deg2rad(pitch_movement_deg)
#                 target_yaw_rad = self.start_yaw_rad - np.deg2rad(yaw_movement_deg)
                
#                 # Apply safety limits
#                 min_pitch = self.start_pitch_rad - self.max_movement_rad
#                 max_pitch = self.start_pitch_rad + self.max_movement_rad
#                 min_yaw = self.start_yaw_rad - self.max_movement_rad
#                 max_yaw = self.start_yaw_rad + self.max_movement_rad
...
#                 try:
#                     self.camera_controller.set_positions(target_pitch_rad, target_yaw_rad)
                    
#                     # Update shared state
#                     current_positions = self.camera_controller.get_positions()
#                     with self.state_lock:
...
#                     self.logger.warning(f"Camera control error: {e}")
                
#                 time.sleep(0.02)  # 50Hz control loop
                
#         except Exception as e:
#             self.logger.error(f"Camera control thread error: {e}")
#         finally:
#             self.logger.info("Camera control thread exiting")
